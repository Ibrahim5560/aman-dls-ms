...
//    }
    ...
    private Long getApplicationReferenceQueryParam(String url) {
        ...
        } else if(url.contains("/api/applications/")) {
            applicationReferenceNoStr = url.substring(url.indexOf("/api/applications/") + 22);
            int nextParamIndex = applicationReferenceNoStr.indexOf('/');
            if (nextParamIndex != -1) {
                applicationReferenceNoStr = applicationReferenceNoStr.substring(0,nextParamIndex);
            }
    ...
    /**
     * Get web service info VM
     *
     * @param webServiceName
     *
     * @return Web Service Info VM
     */
    public WebServiceInfoVM getWebServiceInfo(WebServiceName webServiceName) {

        if(webServiceName == null) {
            throw new SystemException("Web service name can not be empty");
        }

        Optional<WebServiceDTO> webService = webServiceService.findByName(webServiceName);
        if(!webService.isPresent()) {
            throw new SystemException("Web service info not found for name : " + webServiceName);
        }

        return new WebServiceInfoVM(webService.get());
    }
}
