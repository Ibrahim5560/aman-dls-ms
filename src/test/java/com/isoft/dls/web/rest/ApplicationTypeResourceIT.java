...
public class ApplicationTypeResourceIT {
    ...
    @Test
    @Transactional
    public void getAllApplicationTypesByApplicationIsEqualToSomething() throws Exception {
        // Initialize the database
        applicationTypeRepository.saveAndFlush(applicationType);
        Application application = ApplicationResourceIT.createEntity(em);
        em.persist(application);
        em.flush();
        applicationType.addApplication(application);
        applicationTypeRepository.saveAndFlush(applicationType);
        Long applicationId = application.getId();

        // Get all the applicationTypeList where application equals to applicationId
        defaultApplicationTypeShouldBeFound("applicationId.equals=" + applicationId);

        // Get all the applicationTypeList where application equals to applicationId + 1
        defaultApplicationTypeShouldNotBeFound("applicationId.equals=" + (applicationId + 1));
    }
